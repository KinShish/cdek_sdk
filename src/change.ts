...
const service_codes=['INSURANCE','DELIV_WEEKEND','TAKE_SENDER','DELIV_RECEIVER','TRYING_ON','PART_DELIV','INSPECTION_CARGO',
...
export default class Change{
    ...
    changeServices(data:DataForRequest.Service[],res){
        if(data){
            for(let i in data){
                if(!data[i].code){
    ...
    changeItems(data:DataForRequest.Item[],res){
        if(data){
            for(let i in data){
    ...
    changePackages(data:DataForRequest.Package[],res){
        if(data){
            for(let i in data){
    ...
    changeLocation(data:DataForRequest.Location,res){
        if(data){
    ...
    changeThreshold(data:DataForRequest.Threshold[],res){
        if(data){
            for(let i in data){
                if(!data[i].threshold){
    ...
    changePhones(data:DataForRequest.Phone[],res){
        if(data){
            for(let i in data){
                if (!data[i].number) {
    ...
    changeContact(data:DataForRequest.Contact,res){
        if(data){
            if(data.company){
    ...
    changeSeller(data:DataForRequest.Seller,res){
        if(data){
            if(data.name){
                if(data.name.length>255){
